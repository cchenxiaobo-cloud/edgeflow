...
| 云端元数据 | apiserver + etcd | **无真实 K8s 接入**：内存读缓存 + 嵌入式 etcd 写穿持久化（v0.4.0，决策记录 R13）+ REST API 适配（WBS 2.3/2.6，决策记录 R1，REST 适配形态不变） |
...
│     ├─ etcdstore（v0.4.0：嵌入式单成员 etcd，127.0.0.1:12379/12380）           │
│     └─ audit（audit-ledger.jsonl 审计台账）· metrics（Prometheus 五指标）        │
│   （无 apiserver/外部 etcd：嵌入式 etcd 持久化注册台账与设备 Desired；           │
│    Pod 状态与上报属性重启后 ≤1 上报周期自愈）                                    │
└─────────────────────────────────────┬──────────────────────────────────────────┘
...
- 恢复后：重连 → 重新 Register → **周期全量上报**（PodStatus 30s / DeviceReport 周期可配）驱动云端收敛（Absent 终态保留 90s 后云端删除）。v0.4.0 起注册台账与设备 Desired 跨重启保留（etcd 写穿），重启后边缘重连仅翻新瞬态（心跳/Status/Pod/reported）。
...
| R14 | **外部 etcd 模式（方案④，v0.5.0 兑现 R13 ⑥）** | `EDGEFLOW_CLOUDCORE_ETCD_ENDPOINTS`（逗号分隔 http(s) URL）**非空 = 外部模式**：直连共享 etcd 集群，**跳过 embed**（不建数据目录、不占 12379/12380 端口、忽略 DATA_DIR/CLIENT_URL/PEER_URL/QUOTA/COMPACTION/STRICT——显式设置这些 embed 字段时启动期 `Warn`"仅 embed 生效"，不阻断）；TLS 经 `EDGEFLOW_CLOUDCORE_ETCD_TLS_CA`（非空即启用，服务端证书校验）+ `_TLS_CERT/_TLS_KEY`（同设即 mTLS；只设其一 fail-fast）；业务层（registry/devicestatus/HTTP API）**零改动**（v0.4.0 已冻结的 KVStore 接口即替换面）；三存储包装、读路径内存缓存、键空间 `/edgeflow/`、心跳不落盘等既有语义不变。**单写者形态铁律**：v0.5.0 **不支持**多 cloudcore 副本 active-active——心跳/Offline 判活是各副本**内存瞬态**（不落盘），副本 A 会把连在副本 B 上的活节点按 180s+保留期判死，并从共享键空间**删键 + 级联删设备 Desired**（活节点数据丢失，且无信号）；真多活划 v0.6+（etcd lease 心跳方案）。**护栏**：外部模式存在**非回环端点 ∧ 未启用 TLS → 拒绝启动**（逃生门 `EDGEFLOW_CLOUDCORE_ETCD_ALLOW_INSECURE=1` 显式 opt-in，开启时启动期大告警）；TLS 启用 ⇔ 全部端点必须 https（混合 scheme fail-fast）。**故障语义**：连接失败/启动检查（线性一致读元键，至多 3 次尝试、最坏 ≈17s（预算 ≤20s））不过 = **无条件拒绝启动**（clientv3 是懒连接，fail-fast 靠显式探活落实；恒 fail-fast，外部模式无视 STRICT）；运行中断连由 clientv3 **自动指数退避重连**（应用层零重试、零处理）；/healthz **不反映 etcd 连接状态**（有意为之：避免 K8s 批量重启放大故障） | 设计定稿 subagent_01 §1-§8；风险审稿 subagent_03（前置条件 1-3）；迁移/拓扑/鉴权建议见 DEPLOYMENT.md §10.7；限制 L1/L3/L5/L7 登记见 KNOWN-ISSUES.md §5 |
