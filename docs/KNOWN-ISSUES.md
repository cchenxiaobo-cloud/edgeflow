...
## 38. v0.37.0（规则引擎与实时处理 + 数据治理）

**范围**：pkg/rules（模型/评估/治理）+ RuleSync/RuleEvent 协议 + 云端 rules API（9 端点）
+ 边缘装配与 rule_events 台账；契约 42→51、消息 12→14。

**能力**：见 RELEASE-NOTES-v0370 N1（五块：规则模型 / 评估状态机 / 治理过滤器 / 云边通道 / 边缘装配）。

**边界登记（本版新增）**：
- 动作类型仅 `event`（触发事件上行 + 台账）；指令联动/影子写入动作后续版本评估。
- 告警「恢复/清除」事件不做（状态机内部恢复，不产生 cleared 事件）。
- 云端事件为内存 ring（≤500 条 FIFO）：多副本下各副本仅存本副本接收的事件；聚合与
  持久化归档后续评估。
- debounce 稳定判定用严格相等（float 容差版后续评估）；治理策略每 device×property 单条
  （多策略叠加后续评估）。
- 规则包不自动补发离线节点（重连后需重新 sync；自动补发后续评估）。
- ruleID 校验宽松（小写字母/数字/连字符，1-63 字符；不做 K8s 全量风格校验）。
- 规则替换（RuleSync 应用，含同版本重发）重置全部评估状态——pending 重新计时
  （幂等接受但状态清零，罕见路径）。
- 采样管道仅在「有治理策略或规则」时改变行为；无配置时与 v0.36.0 逐字节等价。
- 空库（从未变更）直接 sync：下发 version=0 规则包 → 边缘拒绝（版本须 ≥1）→ 云端 502；
  正常流程（先创建规则再 sync）不受影响（边缘拒绝为预期，edgecore 测试锚覆盖）。

**升级兼容**：无规则/无治理时采集/影子/上报逐字节不变（旧函数保留为兼容入口）；
契约既有 42 端点零改动；消息既有 12 类型零变化；零新依赖；MQTT/OPC-UA/
视频/模型面零触碰；v0240–v0350 冻结测试零改动。

## 39. v0.38.0（边缘时序存储）

**范围**：pkg/tsdb（序列/段存储、查询/聚合、保留/降采样、水位背压、崩溃恢复）
+ cmd/edgecore 装配（opt-in 开关、采样管道 sink、优雅关闭刷盘）；契约零改动。

**能力**：见 RELEASE-NOTES-v0380 N1（六块：存储模型与写入 / 查询与聚合 / 保留与降采样 /
容量水位与背压 / 持久化与崩溃恢复 / 边缘装配）。

**边界登记（本版新增）**：
- 保留/降采样为段粒度：活动段不参与，段内旧点保留到封段后生效。
- 断电窗口 ≤ flush 周期（默认 1s）：活动段缓冲内数据可能丢失（可接受，登记）。
- rollup 桶不保留 first：降采样后 Query 返回桶代表点（Value=avg），与 raw 语义存在差异。
- 无压缩（朴素二进制段文件）；无跨序列 join / 标签过滤（仅精确 metric+tags 查询）。
- 查询为全段扫描（无段级时间裁剪/索引）——10 万点量级查询约 52ms，优化列后续候选。
- 同一数据目录单实例写：跨实例并发写不承诺（实例内并发安全）。
- e2e 基建发现：edgeEnv 的 300ms 上报/调谐周期低于合法下限（1s）被区间校验回退默认
  （30s/5s，历史现象）；v0.38 e2e 局部替换为 1s 规避；统一修 helper（或评估放开下限）
  待后续版本。
- 崩溃残留 `seg-*.dat.tmp`（降采样 rename 前崩溃）：Open 加载忽略（非 .dat 后缀）、
  运行时重试自动覆盖，不主动回收（无害残留）。
- 泄洪删除段失败（磁盘异常等）：内存索引与统计已减、磁盘孤儿文件在下次 Open 扫描时
  回归（重开自愈；极端路径）。
- 历史遗留（非本版引入，后续文档批顺带清理）：DEVELOPMENT-SPEC NFR-02 仍写“42 端点”
  （现 51）；Chart appVersion 停留 v0.25.0；README 版本历史非严格降序。

**升级兼容**：TSDB 关闭（默认）时采集/影子/上报/规则链逐字节不变（tsSink=nil 零行为）；
契约 51 端点/14 消息零改动；零新依赖；MQTT/OPC-UA/视频/模型面零触碰；
v0240–v0350 冻结测试零改动。
