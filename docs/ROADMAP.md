...
|---------|------|------|-------------------|------|
| WBS 1 | 基础架构搭建 | 20 | 无（M0 无依赖） | 🟨 进行中（1.1 骨架大部分完成） |
| WBS 2 | CloudCore 开发 | 63 | WBS 1、WBS 4（⚠️ CloudHub 需消息协议与连接管理） | ⬜ |
| WBS 3 | EdgeCore 开发 | 65 | WBS 1、WBS 4（⚠️ EdgeHub 需消息协议与连接管理） | ⬜ |
| WBS 4 | 云边通信层 | 28 | WBS 1（协议定义在 WebSocket 通道之前，计划 §7.4 关键路径） | ⬜ |
| WBS 5 | 设备映射器框架 | 22 | WBS 1（CRD）、WBS 2.2、WBS 3.5/3.6（⚠️ 数据流：DeviceTwin ← EventBus ← MetaManager） | ⬜ |
| WBS 6 | 应用管理 | 20 | WBS 3（⚠️ Edged/MetaManager 就绪后） | ⬜ |
| WBS 7 | 安全与认证 | 18 | WBS 1；7.4 依赖 WBS 4.5（⚠️） | ⬜ |
| WBS 8 | 测试与部署 | 30 | 随各模块并行；8.5 依赖 WBS 2 产物；8.6 依赖 WBS 4 协议（⚠️） | ⬜ |
| WBS 9 | 文档与示例 | 14 | WBS 1（可全程并行） | ⬜ |
| WBS 10 | 审查补充项 | 24 | WBS 2/3（可观测性）、WBS 8.6（升级回滚，⚠️） | ⬜ |

### 1.2 二级模块明细

| 模块 ID | 名称 | 子任务（计划原文） | 人天 | 依赖（⚠️ 为推断） | 状态 |
|---------|------|-------------------|------|-------------------|------|
| 1.1 | 项目骨架 | Go 项目结构、Makefile、go.mod | 2 | 无 | 🟨 骨架已完成，剩余共享库见首模块 |
| 1.2 | CI/CD 流水线 | GitHub Actions、多架构构建 | 3 | 1.1 | 🟨 基础 CI 已有；缺 lint/覆盖率/多架构 |
| 1.3 | 开发环境 | 本地 K8s 集群、边缘节点模拟 | 2 | 1.1 | ⬜ |
| 1.4 | API 规范定义 | CRD 定义、OpenAPI schema | 3 | 1.1 | ⬜ |
| 1.5 | 共享库与工具包 | 日志、配置、错误处理 | 4 | 1.1 | ⬜ |
| 1.6 | 构建与发布脚本 | 交叉编译、版本管理 | 3 | 1.1 | ⬜ |
| 1.7 | 代码质量基建 | golangci-lint、覆盖率、安全扫描 | 3 | 1.1 | ⬜ |
| 2.1 | CloudHub 模块 | WebSocket 服务端、消息编解码、会话管理、连接池 | 12 | 1.x、4.1、4.2（⚠️） | ⬜ |
| 2.2 | DeviceController | 设备 CRD 控制器、状态同步 | 10 | 1.4、5.3（⚠️ 同一 CRD） | ⬜ |
| 2.3 | EdgeController | 边缘节点注册、状态管理 | 8 | 2.6（⚠️ 需 apiserver 交互） | ⬜ |
| 2.4 | NodeController | 心跳监控、节点上下线 | 6 | 2.3（⚠️ 注册后才有心跳） | ⬜ |
| 2.5 | CloudCore API 层 | RESTful API、认证中间件 | 5 | 2.6（⚠️） | ⬜ |
| 2.6 | 云端元数据存储 | K8s apiserver 交互、缓存层 | 6 | 1.x | ⬜ |
| 2.7 | CloudCore 配置管理 | 动态配置、热重载 | 4 | 1.5（⚠️） | ⬜ |
| 2.8 | 云端任务管理 | NodeJob CRD、任务分发 | 8 | 2.1、2.3（⚠️） | ⬜ |
| 2.9 | 可观测性基建 | 监控指标、日志收集、告警 | 4 | 2.1（⚠️） | ⬜ |
| 3.1 | EdgeHub 模块 | WebSocket 客户端、消息路由、断线重连 | 12 | 1.x、4.1、4.2（⚠️） | ⬜ |
| 3.2 | Edged 模块 | 轻量容器运行时管理、Pod 生命周期 | 30 | ⛔ **Edged 策略 P0 决策**（方案 A/B/C）；3.3（⚠️ 数据流 EdgeHub→MetaManager→Edged） | ⬜ |
| 3.3 | MetaManager | 本地元数据存储(SQLite)、数据同步 | 8 | 1.5（⚠️） | ⬜ |
| 3.4 | 边缘自治引擎 | 断网独立运行、重连后同步、冲突解决 | 15 | 3.2、3.3、4.6（⚠️） | ⬜ |
| 3.5 | DeviceTwin 模块 | 设备影子、状态同步 | 6 | 3.6、3.3（⚠️） | ⬜ |
| 3.6 | EventBus 模块 | MQTT 消息总线 | 4 | ⚠️ NATS MQTT 兼容性 POC（开发前 1-2 天，计划 §4.2.4） | ⬜ |
| 3.7 | ServiceBus 模块 | 边缘服务发现、路由 | 4 | 4.x（⚠️ 云边 HTTP 调用，Phase 3） | ⬜ |
| 3.8 | 可观测性(边缘) | 指标暴露、日志上报 | 6 | 3.1（⚠️） | ⬜ |
| 4.1 | 消息协议设计 | 消息格式(Protobuf)、类型枚举 | 3 | 1.x | ⬜ |
| 4.2 | 连接管理 | 连接池、心跳保活、重连退避 | 6 | 4.1（⚠️） | ⬜ |
| 4.3 | 消息路由引擎 | 主题路由、消息过滤 | 5 | 4.1、4.2（⚠️） | ⬜ |
| 4.4 | 数据压缩与序列化 | 消息压缩、增量同步 | 4 | 4.1（⚠️） | ⬜ |
| 4.5 | 安全传输层 | TLS/MTLS、证书轮换 | 5 | 7.1（⚠️） | ⬜ |
| 4.6 | 消息可靠投递 | ACK、重试、幂等 | 5 | 4.1、4.2（⚠️） | ⬜ |
| 5.1 | Mapper 框架 | Mapper SDK、设备注册/注销 | 6 | 1.4（⚠️） | ⬜ |
| 5.2 | 设备协议适配 | Modbus、OPC-UA、MQTT 适配器 | 5 | 5.1（⚠️） | ⬜ |
| 5.3 | 设备 CRD 控制器 | Device/DeviceModel CRD | 4 | 1.4（⚠️） | ⬜ |
| 5.4 | 设备数据流水线 | 采集→转换→上报→持久化 | 4 | 5.1、3.3（⚠️） | ⬜ |
| 5.5 | 设备示例 Mapper | 完整 Mapper 示例 | 3 | 5.2（⚠️） | ⬜ |
| 6.1 | 边缘应用部署 | Pod 部署、Deployment 支持 | 5 | 3.2（⚠️） | ⬜ |
| 6.2 | 配置下发 | ConfigMap/Secret 同步 | 3 | 3.3、4.x（⚠️） | ⬜ |
| 6.3 | 状态上报 | Pod 状态上报、健康检查 | 3 | 3.2（⚠️） | ⬜ |
| 6.4 | 边缘应用升级 | 滚动更新、回滚 | 4 | 6.1（⚠️） | ⬜ |
| 6.5 | 资源管理 | 边缘调度、资源超卖 | 5 | 6.1（⚠️） | ⬜ |
| 7.1 | 证书管理 | CA 初始化、签发、轮换 | 5 | 1.x | ⬜ |
| 7.2 | RBAC | 角色/权限定义 | 3 | 1.4（⚠️） | ⬜ |
| 7.3 | 设备认证 | 设备身份注册、Token 认证 | 4 | 5.x（⚠️） | ⬜ |
| 7.4 | 云边认证 | MTLS 握手、节点验证 | 3 | 7.1、4.5（⚠️） | ⬜ |
| 7.5 | 审计日志 | 操作审计、安全事件 | 3 | 7.1（⚠️） | ⬜ |
| 8.1 | 单元测试 | 核心模块测试、覆盖率≥70% | 8 | 随各模块并行 | ⬜ |
| 8.2 | 集成测试 | 云边联调、设备管理端到端 | 6 | 2.x、3.x（⚠️） | ⬜ |
| 8.3 | E2E 测试 | 多节点、故障恢复、自治 | 5 | 3.4（⚠️） | ⬜ |
| 8.4 | 性能测试 | 节点规模模拟、延迟测试 | 4 | 8.2（⚠️） | ⬜ |
| 8.5 | Helm Chart | Chart 编写、多环境 values | 3 | 2.x 产物（⚠️） | ⬜ |
| 8.6 | keadm 安装工具 | 安装工具、注册流程、升级 | 4 | 4.x（⚠️ 注册协议） | ⬜ |
| 9.1 | 架构文档 | 设计文档、模块说明 | 3 | 1.x | ⬜ |
| 9.2 | API 文档 | API 参考、CRD 文档 | 2 | 1.4（⚠️） | ⬜ |
| 9.3 | 部署指南 | 快速开始、生产部署 | 3 | 8.5（⚠️） | ⬜ |
| 9.4 | 开发者指南 | 贡献指南、代码规范 | 2 | 1.x | ⬜ |
| 9.5 | 示例与教程 | 设备接入、应用部署示例 | 4 | 5.x、6.x（⚠️） | ⬜ |
| 10.1 | 可观测性基建 | 监控(Prometheus)、日志(Fluent Bit)、告警 | 10 | 2.9、3.8（⚠️ 职责需与 2.9/3.8 对齐） | ⬜ |
| 10.2 | 边缘节点升级与回滚 | OTA 升级、灰度发布 | 6 | 8.6（⚠️） | ⬜ |
| 10.3 | API 兼容性测试 | K8s API 版本兼容矩阵 | 4 | 2.x（⚠️） | ⬜ |
| 10.4 | 安全扫描与修复 | 镜像扫描、依赖漏洞 | 4 | 8.5（⚠️） | ⬜ |
...
| M3 | 设备管理与数字孪生 | **2.2、3.5、3.6、5.1-5.5**、8.2/8.3 设备部分 | CRD 定义 DeviceModel/Device；Twin desired 变更同步并触发设备操作；设备状态上报；MQTT Mapper 读写模拟设备；端到端延迟 ≤5s |
