...
**版本：v0.10.0**（2026-08-26） ｜ 适用产品：EdgeFlow v0.10.0

> 本文档随产品发布，内容与 v0.10.0 开发进展一致；未实现功能已标注"即将上线"或列入附录 E 已知限制。
>
> - LaTeX/PDF 版：`docs/manual/`（EdgeFlow-用户手册-v0.10.0.pdf，91 页）
...
| `EDGEFLOW_CLOUDCORE_RELEASE_MIRROR_CHECK` | 空 | 发布前镜像存在性探活：空/off=不检查（默认）、warn=失败仅告警、fail=失败阻断发布（422）（v0.9.0） |
