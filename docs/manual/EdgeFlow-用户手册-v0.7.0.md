...
**版本：v0.7.0**（2026-08-25） ｜ 适用产品：EdgeFlow v0.7.0

> 本文档随产品发布，内容与 v0.7.0 开发进展一致；未实现功能已标注"即将上线"或列入附录 E 已知限制。
>
> - LaTeX/PDF 版：`docs/manual/`（EdgeFlow-用户手册-v0.7.0.pdf，91 页）
...
| **方法** | **路径** | **说明** | **主要状态码** |
|:---|:---|:---|:---|
| GET | `/api/v1/models` | 模型列表（K8s List 风格，按 name 排序） |  |
...
分桶（按路由模式与状态码）；外部多副本形态下建议关注 etcd
租约续期失败（当前无独立指标，见 KNOWN-ISSUES L12，后续版本规划）。
...
1.  **云端分级持久化（v0.4.0 起）**：注册台账与设备 Desired
    跨重启保留（写穿 etcd）；Pod 状态与上报属性重启后短暂清空 （**≤1
    上报周期自愈，非永久丢失**）；心跳/Status 不落盘
