...
\begin{itemize}
  ...
  \item \textbf{手册修订}：v0.1.1（2026-08-16）——依据仓库代码审计修订（吊销闭环、OCSP、设备认证 token 消费、配置文件与热重载、gzip 压缩、keadm 命令修正等），产品版本基线不变（仍为 v0.1.0）；
  \item \textbf{手册修订}：v0.1.2（2026-08-18）——v0.1.1 生产发布准备轮安全加固（\texttt{/ocsp} per-IP 限流与缓存、OCSP 客户端新鲜度校验、CRL 锁降级日志、P2 代码审查遗留闭环），产品版本基线 v0.1.1；
