...
发布目录 \texttt{release/v0.1.0/} 提供预编译二进制：\texttt{cloudcore}、\texttt{edgecore}、\texttt{keadm} 每组件均含 \texttt{linux-amd64}、\texttt{linux-arm64} 与 \texttt{darwin-arm64} 三个平台版本（共 9 个二进制），以及 Helm Chart 包 \texttt{edgeflow-0.1.0.tgz}。下载后可使用 \texttt{checksums.txt} 校验文件完整性，并将二进制放入 \texttt{PATH} 或仓库 \texttt{bin/} 目录后按需执行。
