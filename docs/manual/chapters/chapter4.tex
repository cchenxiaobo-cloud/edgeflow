...
\begin{tabularx}{\textwidth}{|l|>{\raggedright\arraybackslash}p{4.7cm}|X|>{\raggedright\arraybackslash}p{3.4cm}|}
  ...
  \hline
  POST & \texttt{/ocsp} & OCSP 在线吊销查询（RFC 6960，DER 编码） & 200 / 400 / 500 \\
  \hline
\end{tabularx}
...
\begin{itemize}
  ...
  \item 状态码：200 返回签名后的吊销状态响应（\texttt{good}/\texttt{revoked}/\texttt{unknown}）；400 请求超限、为空或 DER 损坏；500 CA 不可用、吊销记录损坏或响应构建失败（fail-closed，不静默放行）。
\end{itemize}
...
说明：\texttt{/healthz}、\texttt{/metrics} 与 \texttt{/ocsp} 不参与认证，始终可访问，以保证探活、监控采集与在线吊销查询通道可用（\texttt{/ocsp} 响应由 CA 签名，自带可信性）。
