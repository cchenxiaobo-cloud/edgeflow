...
本章内容：介绍 cloudcore 对外提供的全部 REST API 端点——用途、请求示例与响应字段说明，以及监控指标（\texttt{/metrics}）与 API 认证的启用方法。操作人员可通过 curl 或任意 HTTP 客户端调用。
...
说明：\texttt{/healthz} 与 \texttt{/metrics} 不参与认证，始终可访问，以保证探活与监控采集通道可用。
