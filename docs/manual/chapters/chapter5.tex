...
指定；未设置时使用默认约定 \texttt{edge-\allowbreak<主机名>}。
...
\section{edgecore 环境变量配置}

edgecore 全部通过环境变量配置，未设置的项使用默认值（表~\ref{tab:5-env}）。
时长类变量（如调谐/上报周期）合法范围为 \textbf{1s 至 10m}，超出范围或
非法值回退默认值并打印告警。
