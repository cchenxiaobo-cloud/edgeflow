...
证书生命周期管理：\seqsplit{\texttt{keadm cert rotate}} 轮换（备份先行 + 事务化重签，SAN 继承）、\seqsplit{\texttt{keadm cert revoke}} 吊销（\texttt{--node}/\texttt{--serial} 二选一互斥、重复吊销幂等，序列号入 crl.json 并生成/更新 crl.pem）。CRL 离线吊销：crl.json 为序列号来源记录、crl.pem 为派生签名产物（写路径 flock 进程锁 + crl.json$\leftrightarrow$crl.pem 对账自愈），mTLS 握手按 CRL 拒绝已吊销证书（CRL 损坏/签名无效 fail-closed）；OCSP 在线吊销：POST /ocsp（RFC 6960，DER 编码请求/响应、16KiB 请求上限、状态码 200/400/500），与 CRL 同源（均读 crl.json），吊销即时双通道生效。OCSP 服务端已实现，客户端查询库 API 可用但尚未接入生产路径——边缘 mTLS 握手走 CRL 校验，而非 OCSP。
