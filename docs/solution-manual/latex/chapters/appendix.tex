...
v0.1.0 以“容器镜像 + 配置”为载体：模型与推理运行时打包为版本化镜像 $\rightarrow$ podsync 下发 Pod 声明（image + replicas）$\rightarrow$ 边缘 Edged 拉取运行并持续自治（多副本/健康自愈/防漂移）；推理参数经 config-sync 下发并落盘。模型仓库/版本管理与模型灰度发布平台\warn（升级场景的灰度分批已工具化，见附录 C F42），当前以镜像 Tag + 配置参数化作为过渡路径。
...
\textbf{表 C-1：F01--F40、F49 已实现特性 × 四场景归属（双向追溯）}
...
\textbf{表 C-2：规划中/部分实现特性清单（F41--F48）}
...
F42 & 灰度发布 & 部分实现 & 升级分批已工具化（\seqsplit{\texttt{keadm upgrade --batch-size/--pause-between}} + \seqsplit{\texttt{keadm batch --op=upgrade}}，fail-fast）；模型/按比例灰度仍规划中（4.2）\\
