...
\addlinespace[3pt]
模型应用 & F11--F19 & 容器应用部署与自治：多副本、健康自愈、CrashLoopBackOff、镜像漂移检测；模型灰度发布接收端（F42，v0.7.0 已实现：发布器自动下发推理容器声明与模型参数） & 调度/超卖\\
\bottomrule
\end{tabular}}
...
\textbf{\warn}：训练平台与模型评测、完整 RBAC（当前为单 Token 鉴权）、设备级身份认证（节点接入认证已实现）、调度/超卖、Flannel 网络、OPC-UA 接入、Protobuf 编码（gzip 通道压缩已实现）、产品内置镜像安全扫描（发布流程构建期 Trivy 扫描已建立，基线 0 漏洞）。
