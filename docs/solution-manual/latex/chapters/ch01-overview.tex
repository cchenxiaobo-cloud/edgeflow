...
\begin{itemize}
  ...
  \item \textbf{REST API}：对外提供 13 个 HTTP 端点（含健康检查与指标；管理端点 11 个：设备、节点、命令、podsync/config-sync 等）；
...
\midrule
数据采集 & F21--F30 & MQTT/Modbus 设备接入；Mapper 周期采集（Modbus 30s 上报循环 / mock\_sensor 2s）；EventBus MQTT 数据面（QoS1）；设备影子与属性快照；设备命令下行（可靠投递 F05）；DeviceReport 周期上报（30s，可配） & OPC-UA 协议接入；设备认证\\
\addlinespace[3pt]
...
\textbf{v0.1.0 功能范围（已实现）}：WebSocket 云边通道；边缘容器自治（声明式调谐 5s、多副本、健康自愈、CrashLoopBackOff、镜像漂移检测）；设备管理（MQTT/Modbus 接入、设备影子、设备命令）；生产加固（mTLS/Token/审计台账/keadm/Helm/多架构镜像）。

\textbf{\warn}：模型仓库与版本管理、灰度发布、完整 RBAC（当前为单 Token 鉴权）、设备认证、调度/超卖、Flannel 网络、OPC-UA 接入、通道压缩/Protobuf 编码、镜像安全扫描。
