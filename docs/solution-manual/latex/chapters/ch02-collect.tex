...
\begin{itemize}
  ...
  \item \textbf{影子汇聚}：DeviceTwin 影子自动创建，合并 desired/reported（F21），边缘影子始终为最新值；设备属性按周期上报云端（F22），云端视图由上报构建（属性快照为云端内存态；v0.4.0 起注册台账与设备 Desired 已云端持久化，见第 1 章“云端部署形态与高可用”节）。
  \item \textbf{OPC-UA（v0.3.0 起）}：已交付 UA Binary 协议栈核心（NodeId/Variant/DataValue/DateTime 等类型编解码 + HEL/ACK 握手，纯标准库、零第三方依赖）；\textbf{未实现}设备读写服务（Read/Write/Subscribe）、SecureChannel、安全策略（当前仅 SecurityPolicy None 明文）、Mapper 接入，未与 open62541/node-opcua 互操作验证——\textbf{仅限可信隔离网络，严禁暴露公网}；接入真实 OPC-UA 设备排后续版本。
...
\begin{itemize}
  ...
  \item 云端分级持久化（v0.4.0 起）：节点注册台账与设备 Desired 跨重启保留（嵌入式 etcd 写穿，见第 1 章“云端部署形态与高可用”节）；设备属性快照仍为内存态，云端重启后 $\leq$1 上报周期自愈（详见第 3 章）。
\end{itemize}
