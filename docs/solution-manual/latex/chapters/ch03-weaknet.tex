...
\env{EDGEFLOW\_EDGECORE\_DEVICE\_REPORT\_INTERVAL} & 30s（1s--10min） & 设备状态上报周期，弱网可调大降低流量（F22）\\
\env{EDGEFLOW\_EDGECORE\_REPORT\_INTERVAL} & 30s（1s--10min） & Pod 状态上报周期（F18）\\
...
云端重启 & 云端内存态清空；边缘读超时约 90s $\rightarrow$ 退避重连 $\rightarrow$ 自动重注册 $\rightarrow$ 周期上报重建 & 自动，无需人工（E2E 实测） & 云端无持久化恢复，依赖边缘重建\\
