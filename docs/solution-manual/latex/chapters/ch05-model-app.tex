...
\textbf{总体思路}：将推理服务以容器化形式部署于边缘节点，与数据源同侧运行（\textbf{近数据侧推理}），构建“数据供给 $\rightarrow$ 推理消费 $\rightarrow$ 结果闭环”的本地推理链路；模型由第 4 章模型管理流程分发——v0.7.0 起即\textbf{模型仓库注册 $\rightarrow$ 版本激活 $\rightarrow$ 灰度发布}（发布器自动下发推理容器声明与模型参数），v0.1.0 时代的“镜像 + 配置”手工通道仍可用（兼容路径，见 4.4.3）。
...
\textbf{（1）推理服务声明}（YAML 风格，字段示意；完整声明格式以产品文档为准。v0.7.0 起推理容器声明由灰度发布器自动下发（镜像 = 版本 mirror，见 4.4.1），也可沿用 podsync 手工声明）：
