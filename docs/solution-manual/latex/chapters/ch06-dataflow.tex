...
6 & 云端接收 $\rightarrow$ 存储 & DeviceStatus 快照 & CloudCore 内存态（v0.4.0 起注册台账与设备 Desired 已持久化，属性快照仍内存态，见第 1 章“云端部署形态与高可用”节） & nodeID $\rightarrow$ namespace/deviceName $\rightarrow$ 状态\\
...
\begin{itemize}
  ...
  \item \textbf{云端视角}：云端 DeviceStatus 为最新状态快照（属性快照为内存态；v0.4.0 起节点注册台账与设备 Desired 已持久化，云端重启后台账保留、节点短暂 Unknown，见第 1 章“云端部署形态与高可用”节）。云边断连期间，云端展示的是最后一次成功收到的 DeviceReport；连接恢复后，边缘按 30s 周期（可配 1s--10min）补报最新快照，且每次边缘启动即上报一轮；
