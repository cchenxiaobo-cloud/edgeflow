...
// ContractEndpoints 是 cloudcore HTTP 端点契约表（38 条 = 既有 14 + v0.7.0 模型 API 17 + v0.12.0 digest 复核 1 + v0.16.0 pause/resume/export/import 4 + v0.17.0 PATCH 可调参数 1 + v0.18.0 全局部署影子查询 1）。
//
...
var ContractEndpoints = []Endpoint{
	...
	{Method: "POST", Path: "/api/v1/models/{modelName}/releases/{releaseID}/pause", Note: "暂停发布（v0.16.0；running→paused，节点边界生效）"},
	...
	{Method: "GET", Path: "/api/v1/deployments", Note: "全局部署影子查询（v0.18.0；跨模型聚合，model/nodeID 过滤可选；另 v0.18.0：失败预算 failureBudget 自动暂停、发布事件时间线 events，均不新增端点）"},
